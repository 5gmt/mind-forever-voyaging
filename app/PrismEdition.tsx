  ...
  return (
    <main className="prism-edition" data-era={displayYear ?? "system"} data-phase={phase} data-mode={(mode || "opening").replace(" Mode", "").toLowerCase()} data-context-open={contextOpen} data-contrast={highContrast ? "high" : "standard"} data-reduce-motion={reduceMotion} data-qa={qaEnabled ? "true" : "false"}>
      ...
      <section className="experience-shell" aria-label="Interactive story" aria-hidden={blockingOverlayOpen} inert={blockingOverlayOpen ? true : undefined}>
        ...
        <section className="command-deck" lang={locale} aria-label={uiText(locale, "storyControls")}>
          ...
          {!assistedSecurity && !assistedYearSelector && assisted && inputKind === "line" && mode === "Communications Mode" && outlets.length > 0 && <section className="outlet-switcher" aria-label={uiText(locale, "communicationOutlets")}>
            ...
            <div>{outlets.map((outlet) => <button type="button" key={outlet.code} className={activeOutletCode === outlet.code ? "active" : ""} onClick={() => sendCommand(outlet.code)} disabled={!acceptsInput}><span>{outlet.code}</span><strong>{localizeOutletLabel(locale, outlet.code, outlet.name)}</strong></button>)}</div>
          </section>}
