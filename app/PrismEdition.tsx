...
import { openingPresentation, type BridgePresentationLine } from "./story-presentation";
...
export default function PrismEdition() {
  ...
  const [presentationLines, setPresentationLines] = useState<BridgePresentationLine[]>([]);
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      ...
      setPresentationLines(event.data.presentation?.version === 1 && Array.isArray(event.data.presentation.lines)
        ? event.data.presentation.lines
        : []);
  ...
  const presentedOpening = useMemo(
    () => locale === "ja" ? openingPresentation(presentationLines, locale) : null,
    [locale, presentationLines],
  );
