...
import { localizeOutletLabel, uiText, type Locale, type UiKey } from "./localization";
...
export default function PrismEdition() {
  ...
  const [aliasNotice, setAliasNotice] = useState<CommandNotice | null>(null);
  ...
  const parserCoachKey: UiKey = /i don['’]t know the word|not a verb i recognise|not a verb i recognize/i.test(latestSceneText)
    ? interactionLevel === "classic" ? "shorterNounHelp" : "shorterCommandHelp"
    : /you['’]ll have to be more specific|which .* do you mean/i.test(latestSceneText)
      ? interactionLevel === "classic" ? "specificNounHelp" : "exactObjectHelp"
      : /you can['’]t (?:see|hear) any|not here/i.test(latestSceneText)
        ? "absentObjectHelp"
        : "commandHistoryHelp";
  const commandHelp = aliasNotice
    ? uiText(locale, aliasNotice.key).replace(aliasNotice.key === "understoodAs" ? "{command}" : "{destination}", aliasNotice.value ?? "")
    : uiText(locale, parserCoachKey);
  ...
  const sendCommand = (raw: string) => {
    ...
    setAliasNotice(normalized.toLowerCase() !== raw.trim().toLowerCase() ? { key: "understoodAs", value: normalized } : null);
  ...
  const draftSceneCommand = (value: string) => {
    setCommand(value);
    setAliasNotice({ key: "draftedFromPassage" });
  ...
  const useMapRouteStep = () => {
    ...
    if (interactionLevel === "guided") {
      ...
      setAliasNotice({ key: "routeDrafted", value: mapRoutePreview.destination.label });
      setFieldworkOpen(false);
      window.setTimeout(() => commandRef.current?.focus(), 40);
    } else {
    ...
    <main className="prism-edition" data-era={displayYear ?? "system"} data-phase={phase} data-mode={(mode || "opening").replace(" Mode", "").toLowerCase()} data-context-open={contextOpen} data-contrast={highContrast ? "high" : "standard"} data-reduce-motion={reduceMotion} data-qa={qaEnabled ? "true" : "false"}>
      ...
      <section className="experience-shell" aria-label="Interactive story" aria-hidden={blockingOverlayOpen} inert={blockingOverlayOpen ? true : undefined}>
        <header className="console-header">
          <div className="location-block" lang={locale} aria-live="polite"><span className="section-kicker">{phaseLabel[phase]}</span><strong>{currentPlace || (phase === "origin" ? "Personal archive" : phase === "comparative" ? "Simulation archive" : phase === "witness" ? "Review channel" : phase === "lockdown" ? "Restricted system" : phase === "epilogue" ? "A final voyage" : mode ? guide.label : uiText(locale, "openingTransmission"))}</strong><span>{displayYear ? `${displayYear} · ${currentPlace || "Rockvil"}` : activeOutlet ? `OUTLET ${activeOutlet.code}` : phase === "origin" ? "Memory files" : phase === "comparative" ? "Visited horizons" : phase === "witness" ? "Findings received" : phase === "lockdown" ? "External control detected" : phase === "epilogue" ? "Memory continuing" : mode === "Simulation Mode" ? "Locating…" : discovery.identityKnown ? "Project date · 2031" : uiText(locale, "carrierLocked")}</span></div>
          {phase === "lockdown" && <div className="lockdown-banner">CHANNELS RESTRICTED</div>}
          <div className="system-state" aria-label={uiText(locale, acceptsInput ? "storyReady" : "storyProcessing")}><span className={acceptsInput ? "state-light ready" : "state-light"}></span>{systemActivity}</div>
        ...
        {accessOpen && <section className="access-panel" lang={locale} aria-label={uiText(locale, "readingSettings")}>
          <div><span>{uiText(locale, "language")}</span><div className="segmented"><button type="button" lang="en" className={locale === "en" ? "active" : ""} aria-pressed={locale === "en"} onClick={() => setLocale("en")}>English</button><button type="button" lang="ja" className={locale === "ja" ? "active" : ""} aria-pressed={locale === "ja"} onClick={() => setLocale("ja")}>日本語</button></div></div>
        ...
        <section className="command-deck" aria-labelledby="story-controls-label">
          ...
          {!assistedSecurity && !assistedYearSelector && assisted && inputKind === "line" && mode === "Communications Mode" && outlets.length > 0 && <section className="outlet-switcher" lang={locale} aria-label={uiText(locale, "communicationOutlets")}>
          ...
          {!assistedSecurity && !assistedYearSelector && !yesNoPrompt && (inputKind === "char" ? <div className="character-prompt">
            {mode === "Library Mode" ? <><span className="section-kicker">Character menu active</span><div className="library-controls">{[["Previous", "p"], ["Next", "n"], ["Open", "o"], ["Read", "r"], ["Close", "c"], ["Exit", "e"]].map(([label, value]) => <button type="button" key={value} onClick={() => postCommand(value, true)} disabled={!acceptsInput}>{label}<kbd>{value.toUpperCase()}</kbd></button>)}</div></> : <button className="continue-button" lang={locale} type="button" onClick={() => postCommand(" ", true)} disabled={!acceptsInput}><span>{phase === "signal" ? (locale === "ja" ? "原作を始める" : "Begin the original story") : uiText(locale, "continue")}</span><strong>{uiText(locale, "pressAnyKey")}</strong></button>}
          </div> : <>
            <form onSubmit={submitCommand} className="command-form" lang={locale}>
            ...
            {assisted && <div className="quick-actions" lang={locale} aria-label={uiText(locale, "commonActions")}>{baseActions.map(([label, value]) => <button type="button" key={`${mode}-${label}`} onClick={() => sendCommand(value)} disabled={!acceptsInput} lang={["look", "display outlets"].includes(value) ? locale : "en"}>{label}</button>)}</div>}
      ...
      <aside className="companion-panel" aria-label={uiText(locale, "readerCompanion")} aria-hidden={blockingOverlayOpen || !contextOpen} inert={blockingOverlayOpen ? true : undefined}>
        <div className="companion-tabs" lang={locale} role="tablist" aria-label={uiText(locale, "companionViews")} style={{ "--tab-count": panelTabs.length } as React.CSSProperties}>{panelTabs.map((panel) => <button key={panel.id} type="button" role="tab" aria-selected={activePanel === panel.id} onClick={() => setActivePanel(panel.id)}>{panel.label}</button>)}<button className="close-context" type="button" onClick={() => setContextOpen(false)} aria-label={uiText(locale, "closeCompanion")}>×</button></div>
        <div className="companion-content">
          {activePanel === "guide" && <section className="companion-section" lang={locale}><span className="section-kicker">{uiText(locale, "help")}</span><h2>{guide.label}</h2><p>{guide.copy}</p><div className="play-style-picker" aria-label={uiText(locale, "choosePlayControls")}>{([['classic', 'classic', 'guideClassicDescription'], ['guided', 'guided', 'guideGuidedDescription'], ['actions', 'actionMenus', 'introActionMenusDescription']] as Array<[InteractionLevel, UiKey, UiKey]>).map(([value, labelKey, copyKey]) => <button type="button" key={value} aria-pressed={interactionLevel === value} className={interactionLevel === value ? "active" : ""} onClick={() => chooseInteractionLevel(value)}><strong>{uiText(locale, labelKey)}</strong><span>{uiText(locale, copyKey)}</span></button>)}</div><div className="purpose-loop"><span>{uiText(locale, "notice")}</span><i>→</i><span>{uiText(locale, "act")}</span>{discovery.simulationEntered && <><i>→</i><span>RECORD</span></>}{visitedYears.length >= 2 && <><i>→</i><span>COMPARE</span></>}{discovery.evidenceAccepted && <><i>→</i><span>DECIDE</span></>}{discovery.lockdown && <><i>→</i><span>TRANSMIT</span></>}</div><div className="guide-callout"><span>01</span><p>{uiText(locale, interactionLevel === "classic" ? "classicCallout" : interactionLevel === "guided" ? "guidedCallout" : "actionMenusCallout")}</p></div><div className="guide-callout"><span>02</span><p>{uiText(locale, "lookInventoryHelp")}</p></div><div className="guide-callout"><span>03</span><p>{uiText(locale, "storySafeHelp")}</p></div><details><summary>{uiText(locale, "interactiveFictionHelp")}</summary><p>{uiText(locale, "interactiveFictionHelpCopy")}</p></details><details><summary>{uiText(locale, "stuckHelp")}</summary><p>{uiText(locale, "stuckHelpCopy")}</p></details></section>}
          ...
          {activePanel === "context" && (!mode || mode === "Sleep Mode") && <section className="companion-section" lang={mode === "Sleep Mode" ? "en" : locale}><span className="section-kicker">{mode === "Sleep Mode" ? "Sleep Mode" : uiText(locale, "incoming")}</span><h2>{mode === "Sleep Mode" ? "Background processing" : uiText(locale, "openingTransmission")}</h2><p>{mode === "Sleep Mode" ? "Time is passing. Return to Communications Mode when you are ready." : uiText(locale, "openingContextCopy")}</p></section>}

          {activePanel === "package" && <section className="companion-section package-section" lang={locale}><span className="section-kicker">{uiText(locale, "originalBoxContents")}</span><h2>{uiText(locale, "packageHeading")}</h2><p>{uiText(locale, "packageCopy")}</p><div className="package-shelf"><button type="button" onClick={() => setPackageItem("map")}><img src="/package/rockvil-map-back.jpg" alt="" /><span><strong>{uiText(locale, "rockvilMap")}</strong><small>{uiText(locale, "streetMap2031")}</small></span></button><button type="button" onClick={() => setPackageItem("decoder")}><img src="/package/security-decoder.jpg" alt="" /><span><strong>{uiText(locale, "securityDecoder")}</strong><small>{uiText(locale, "classOneWheel")}</small></span></button><button type="button" onClick={() => setPackageItem("manual")}><span className="manual-thumb">D/O</span><span><strong>{uiText(locale, "dakotaManual")}</strong><small>{uiText(locale, "originalScannedPdf")}</small></span></button></div></section>}
          ...
          {activePanel === "about" && <section className="companion-section about-section" lang={locale}><span className="section-kicker">{uiText(locale, "companionAbout")}</span><h2 lang="en">A Mind Forever Voyaging</h2><p>{uiText(locale, "aboutCopy")}</p><p>{uiText(locale, "aboutParserCopy")}</p><details><summary>{uiText(locale, "criticalContext")}</summary><p>{uiText(locale, "criticalContextCopy")}</p></details><dl className="provenance-list"><div><dt>{uiText(locale, "story")}</dt><dd>{uiText(locale, "releaseDate")}</dd></div><div><dt>{uiText(locale, "runtime")}</dt><dd>Parchment 2026.8.1</dd></div><div><dt>{uiText(locale, "integrity")}</dt><dd>SHA-256 14e2fd18…511216d</dd></div><div><dt>{uiText(locale, "source")}</dt><dd>{uiText(locale, "preservedZil")}</dd></div></dl><a href="https://github.com/the-infocom-files/amfv" target="_blank" rel="noreferrer">{uiText(locale, "historicalSource")}</a><a href="https://github.com/curiousdannii/parchment" target="_blank" rel="noreferrer">{uiText(locale, "parchmentAbout")}</a><div className="debug-entry"><span className="section-kicker">For teachers, testers & the curious</span><h3>Spoiler / debug mode</h3><p>Load a separate QA story with Infocom’s dormant developer shortcuts restored. It can jump to later acts while the live canonical interpreter waits exactly where you left it.</p>{qaEnabled ? <button type="button" onClick={returnToCanonical}>Return to canonical Release 79</button> : <button type="button" onClick={() => setQaWarningOpen(true)}>Open spoiler/debug tools</button>}</div><p className="copyright-note">{uiText(locale, "copyrightNote")}</p></section>}
      ...
      {shareState !== "idle" && <div className={`share-toast ${shareState}`} role="status" lang={locale}>{uiText(locale, shareState === "shared" ? "shared" : shareState === "copied" ? "linkCopied" : "linkCopyFailed")}</div>}

      {introOpen && <section className="intro-overlay" lang={locale} role="dialog" aria-modal="true" aria-labelledby="intro-title" aria-hidden={Boolean(packageItem)} inert={packageItem ? true : undefined}>
        ...
        <div className="intro-content">
          ...
          <div className="intro-actions"><button type="button" className="begin-button" onClick={begin}>{returning ? uiText(locale, "introReturn") : `${uiText(locale, "introBegin")} · ${uiText(locale, interactionLevel === "classic" ? "classic" : interactionLevel === "guided" ? "guided" : "actionMenus")}`}<span>→</span></button><button type="button" className="package-button" onClick={() => setPackageItem("map")}>{uiText(locale, "introOpenPackage")}</button><button type="button" className="share-intro-button" onClick={handleShare}><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="18" cy="5" r="2.4" /><circle cx="6" cy="12" r="2.4" /><circle cx="18" cy="19" r="2.4" /><path d="m8.1 10.8 7.7-4.5M8.1 13.2l7.7 4.5" /></svg>{uiText(locale, "shareEdition")}</button></div>
          <details className="content-note"><summary>{uiText(locale, "introContentNote")}</summary><p>{uiText(locale, "introContentWarning")}</p></details>
          <p className="intro-credit">{uiText(locale, "introCredits")}</p>
        </div>
      </section>}
      ...
      <div className="screen-reader-status" lang={locale} aria-live="polite" aria-hidden={introOpen || qaWarningOpen}>{mode === "Communications Mode" ? uiText(locale, "communicationsAnnouncement") : mode ? `${mode}.` : uiText(locale, "storyOpeningAnnouncement")} {currentPlace || ""} {displayYear || ""}</div>
