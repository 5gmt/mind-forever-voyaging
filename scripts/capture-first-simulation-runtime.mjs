...
const output = argument("--output", "/tmp/parchment-first-simulation-runtime-observed.json");
const commandPath = ["PEOF", "WAIT", "WAIT", "WAIT", "WAIT", "ENTER SIMULATION MODE", "<observed decoder answer>", "LOOK", "RECORD", "WAIT", "RECORD OFF"];
...
try {
  ...
  const fixture = {
    ...
    path: commandPath,
    stableIdentity: {
      assignmentsBoundary: { startsAfter: "A message begins coming over the message line:", endsBefore: "By the way, since the Simulation Controller", count: 9 },
      securityShell: "Simulation Mode is a Class One Security mode. For access, enter the Security Code corresponding to:",
      firstSimulation: { yearsHence: 10, location: "Kennedy Park" },
    },
    dynamicFields: ["security color", "security inner number", "derived canonical numeric answer", "first-simulation month/day", "first-simulation time"],
    sessions,
  };
  await writeFile(output, `${JSON.stringify(fixture, null, 2)}\n`);
  process.stdout.write(`Captured two fresh sessions to ${output}\n`);
} finally {
  await browser.close();
}
